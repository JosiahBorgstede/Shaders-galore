...
void SceneNode::Update(glm::mat4 projectionMatrix, Camera* camera){
    if(m_object!=nullptr){
        ...
        currModel = m_worldTransform.GetInternalMatrix();
        currView = camera->GetWorldToViewmatrix();
        currProj = projectionMatrix;
        ...
        m_shader.SetUniformMatrix4fv("oldmodel", &prevModel[0][0]);
        m_shader.SetUniformMatrix4fv("oldview", &prevView[0][0]);
        m_shader.SetUniformMatrix4fv("oldprojection", &prevProj[0][0]);
